# app/gemini_analyzer.py - Versão Mestre Final
...
**ÁRVORE DE DECISÃO:**
1.  **É LIXO?** O propósito principal da mensagem é um resumo de resultados (com muitos ✅/❌), um comentário, ou um meme/GIF? - Se sim, responda `{"bet_type": "TRASH"}`.
2.  **É UMA ESCADA (LADDER)?** Mostra o mesmo mercado/jogador com linhas crescentes (ex: Chutes 2+, 3+)? - Se sim, responda `{"bet_type": "LADDER"}`.
...
- `unidade`: É CRÍTICO. DEVE ser um número float. Procure por '%' ou 'u'. "3%" -> 3.0.
...
# PROMPT MULTI_SIMPLE ATUALIZADO
...
- Se um item da lista for uma 'Múltipla' genérica, extraia-a também, preenchendo os campos de `jogos`, `descricao` e `entrada` com 'Múltipla' e `tipo_de_aposta` como 'MÚLTIPLA'.
