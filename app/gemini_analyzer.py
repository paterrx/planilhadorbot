...
# --- PROMPTS DOS ESPECIALISTAS E REVISORES (VERSÃO FINALÍSSIMA) ---
...
**ÁRVORE DE DECISÃO:**
1.  **É LIXO?** O propósito principal da mensagem é um resumo de resultados (com muitos ✅/❌), um comentário sobre um jogo, ou um meme/GIF?
...
4.  **É UMA DUPLA/TRIPLA?** A mensagem menciona explicitamente "Dupla" ou "Tripla" e lista 2 ou 3 jogos diferentes?
    - **Sim:** Responda `{"bet_type": "MULTI_SIMPLE"}` (será tratado pelo especialista de múltiplas).
5.  **É UMA LISTA DE SIMPLES (MULTI_SIMPLE)?** A mensagem mostra uma lista de apostas para JOGOS DIFERENTES?
    - **Sim:** Responda `{"bet_type": "MULTI_SIMPLE"}`.
6.  **SE NENHUM DOS ANTERIORES,** é uma aposta única e direta?
...
- `tipo_de_aposta`: DEVE ser 'SIMPLES' para cada item, a menos que esteja escrito 'Múltipla' (nesse caso, use 'MÚLTIPLA').
Conteúdo:
Texto: \"{{message_text}}\"
"""

PROMPT_DUPLA_EXTRACTOR = f"""
Você é um especialista em extrair uma APOSTA DUPLA. Retorne uma LISTA com UM ÚNICO objeto JSON.
{EXTRACTOR_RULES}
REGRAS PARA DUPLA:
...
PROMPT_TRIPLA_EXTRACTOR = f"""
Você é um especialista em extrair uma APOSTA TRIPLA. Retorne uma LISTA com UM ÚNICO objeto JSON.
{EXTRACTOR_RULES}
REGRAS PARA TRIPLA:
...
PROMPT_QA_REFINER_2 = """
Você é o Revisor Chefe. Esta é a prova real final. Uma IA júnior já revisou este JSON. Sua tarefa é fazer uma última verificação rigorosa contra a mensagem original para garantir 100% de precisão antes de ir para a planilha. Corrija qualquer detalhe restante. Sua resposta deve ser apenas o objeto JSON final e perfeito.
**Mensagem Original:** ```{message_text}```
...
PROMPT_MAP = {
    ...
    'DUPLA': PROMPT_DUPLA_EXTRACTOR,
    'TRIPLA': PROMPT_TRIPLA_EXTRACTOR
}