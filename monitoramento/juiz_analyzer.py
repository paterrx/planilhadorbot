...
async def resolve_bet_with_gemini(bet, statistics_json):
    ...
    try:
        ...
        if result_json and isinstance(result_json, dict) and result_json.get('status') in ['Green', 'Red', 'Pendente']:
            status = result_json.get('status')
            logging.warning(f"✅ Veredito do Juiz: {status}!")
            return status
        else:
            logging.error(f"Juiz não conseguiu determinar o resultado para a aposta: {descricao} | {entrada}")
            return "Pendente"
